...
for name in filenames:
    ...
    pca = PCA()
    ann = MLPClassifier(max_iter = 50000)
    clf = BaggingClassifier(base_estimator=ann, bootstrap = True)
    pipe = Pipeline(steps = [('pca',pca),('clf',clf)])
    param_grid = {'pca__n_components' : [20, 30, 40, 50, 60, 70, 80, 90, 100],
                  'clf__base_estimator__hidden_layer_sizes' : [(110), (120),(130),(140), (150),(160),(170), (180)],
                  'clf__n_estimators' : [110, 120, 130, 140, 150, 160, 170, 180]}
    search = GridSearchCV(pipe, param_grid, cv = 10, n_jobs = -1, return_train_score = True)
    search.fit(ts_train, label)
    write_file = dire + 'grid.txt'
